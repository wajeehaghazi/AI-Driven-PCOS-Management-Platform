...
import { Droplets, TestTube, Beaker, AlertCircle, Clock, Shield } from "lucide-react"
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6">
        <header className="text-center mb-8 sm:mb-12">
          <article className="bg-white rounded-lg shadow-sm p-4 sm:p-6 max-w-5xl mx-auto">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900 mb-4">Book an appointment - Message</h2>
            <form
              className="space-y-3"
              onSubmit={(e) => {
                e.preventDefault()
        ...
        <section className="max-w-5xl mx-auto mb-12 sm:mb-16">
          <article className="bg-white rounded-lg shadow-sm overflow-hidden">
            <header className="bg-gradient-to-r from-teal-500 to-blue-500 text-white p-4 sm:p-6 flex items-center gap-3">
              ...
              <h2 className="text-xl sm:text-2xl font-semibold">Sample Collection Request Form</h2>
            </header>
              ...
              <form onSubmit={handleSubmit} className="space-y-8 sm:space-y-10">
                <fieldset className="space-y-4 sm:space-y-6">
                  <legend className="flex items-center gap-3">
                    ...
                    <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Select Test Type</h3>
                  </legend>
                  ...
                  <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                    {testTypeOptions.map((option, index) => (
                      <label
                        key={option.value}
                        className={`border-2 border-gray-200 rounded-lg p-3 sm:p-4 hover:border-teal-300 transition-colors flex items-center space-x-3 cursor-pointer ${index === 2 ? "sm:col-span-2 lg:col-span-1" : ""}`}
                      >
                        <input
                          type="radio"
                          name="testType"
                        ...
                        <span className="flex-1">
                          <strong className="block text-gray-900 text-sm sm:text-base">{option.label}</strong>
                ...
                <fieldset className="space-y-4 sm:space-y-6">
                  ...
                  <section className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                    ...
                    <label className="space-y-2 mb-4">
                      <span className="text-sm font-medium text-gray-700 mb-2 block">Date of Birth</span>
                      <input
                        type="date"
                ...
                <fieldset className="space-y-4 sm:space-y-6">
                  <legend className="flex items-center gap-3">
                    ...
                    <h3 className="text-lg sm:text-xl font-semibold text-gray-900">Collection Details</h3>
                  </legend>

                  <section className="space-y-4 sm:space-y-6">
                    <label className="space-y-2 mb-4">
                      <span className="text-sm font-medium text-gray-700 mb-2 block">Collection Address*</span>
                      <input
                        type="text"
                    ...
                    <section className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                      <label className="space-y-2 mb-4">
                        <span className="text-sm font-medium text-gray-700 mb-2 block">Preferred Date*</span>
                        <input
                          type="date"
                      ...
                      <label className="space-y-2 mb-4">
                        <span className="text-sm font-medium text-gray-700 mb-2 block">Preferred Time*</span>
                        <select
                ...
                <aside className="bg-teal-50 border border-teal-200 rounded-lg p-4 sm:p-6">
                  <label className="flex items-start space-x-3 cursor-pointer">
                    ...
                    <span className="text-xs sm:text-sm text-teal-800 leading-relaxed">
                      By submitting this form, you agree to our terms of service and privacy policy. A medical
                      professional will contact you to confirm your appointment.
                    </span>
                  </label>
                </aside>
 ...
 <section className="mb-12 sm:mb-16 ">
          <header className="text-center mb-8 sm:mb-12">
            ...
            <p className="text-gray-600 max-w-4xl mx-auto text-base sm:text-lg leading-relaxed px-4">
              Follow these step-by-step instructions to properly collect your samples for accurate testing and optimal
              results.
            </p>
          </header>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-4 ">
            {sampleTests.map((test, index) => {
              ...
              return (
                <article
                  key={test.id}
                  className={`bg-white rounded-lg shadow-md overflow-hidden flex flex-col ${index === 2 ? "md:col-span-2 lg:col-span-1" : ""}`}
                >
                  <header className={`${test.bgColor} text-white p-4 sm:p-6 flex items-center gap-3`}>
                    <IconComponent className="w-5 h-5 sm:w-6 sm:h-6 flex-shrink-0" />
                    <h3 className="text-lg sm:text-xl font-semibold">{test.title}</h3>
                  </header>
                  ...
                  <section className="p-4 sm:p-3 sm:py-6 flex-1 flex flex-col ">
                    ...
                    <aside
                      className={`${test.bgLight} border-l-4 ${test.bgColor.replace("bg-", "border-")} rounded-r p-3 sm:p-4 mt-6 flex items-start gap-3`}
                    >
                      <WarningIconComponent
                        className={`w-4 h-4 sm:w-5 sm:h-5 ${test.iconColor} flex-shrink-0 mt-0.5`}
                      />
                      <p className={`${test.textColor} text-xs sm:text-sm font-medium`}>{test.warning}</p>
                    </aside>
                  </section>
                </article>
              )
